...
### PCA loadings
Each PC can be imagined as a sort of meta-gene (a linear combination of the genes included in RunPCA: the variable features) for which every cell has an expression value. The top genes associated with a PC component can be plotted for a selected dimension(s) using the `VizDimLoadings` function.
```{r viz_pca}
